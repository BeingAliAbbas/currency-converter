...
<div class="col-md-4  col-sm-12 col-xs-12">
  <div class="form-group">
    <label><?= lang("price_per_1000") ?> (<?= $sign_convert ?>)</label>
    ...
    if (!empty($service->price) && $user->custom_rate > 0) {
      ...
      $new_price = (!empty($service->price)) ? (float) $service->price : '';
    }
    ?>
    <?php
    if ($currency == 1) {
      $price_show = $service->price/76;
    } else {
      $price_show = $service->price;
    }
    ?>
    <input class="form-control square" name="service_price" type="hidden" value="<?= $new_price ?>">
    <input class="form-control square" type="text" name="service_price" value="<?php echo $sign_convert; ?><?= (!empty($price_show)) ? currency_format($price_show, get_option("currency_decimal")) : '' ?>" readonly>
  </div>
</div>
