...
// $_SESSION['currency'] = get_field(USERS, ["id" => session('uid')], 'currency');
...
<?php 
   if($currency == 1){
    $sign = 'class="fa fa-rupee"';
   }else{
    $sign = 'class="fa fa-dollar"';
   }
?>
<script>
  function myCurrency() {
  document.getElementById("currency_sub").click();
}
function myCurr() {
  document.getElementById("currency_submit").click();
}
</script>
    ...
    <div class="d-flex">
      ...
      <div class="d-flex order-lg-2 ml-auto my-auto">
        ...
        <div class="dark-light">
          <a id="currency" class="link-switcher" onclick="myCurrency()" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Change currency">
          <i <?php print_r($sign); ?>></i>
          </a>
        </div>
       ...
       <form id="currency_form" method="POST">
        <div class="form-group">
                    <label class="switch">
                      ...
                      <input id="currency_sub" onclick="myCurr()" type="checkbox" name="currency" class="custom-switch-input" <?=($currency == 1) ? "checked" : ""?> value="1" hidden>
                      <span class="slider"></span>
                      <input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>">
                      <input id="currency_submit" type="submit" name="currency_submit" value="">
                    </label>
                  </div>
         </form>
    ...
    <div class="row align-items-center">
      ...
      <div class="col-lg order-lg-first">
        <ul class="nav nav-tabs border-0 flex-column flex-lg-row">
          ...
          <li class="nav-item">
            <a href="javascript:void(0)" class="nav-link  <?=(segment(1) == 'dripfeed' || segment(1) == 'order' || segment(1) == 'refill' || segment(1) == 'subscriptions')?"active":""?>" data-toggle="dropdown"><i class="fe fe-edit"></i><?=lang('Order')?></a>
